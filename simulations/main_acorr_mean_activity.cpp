...
int main(int argc,char *argv[])
{
	...
	if(argc == 1) {
		...
		naverage = stoi(argv[1]);
		seed = stoi(argv[2]);

	} else {
		cerr << argv[0] << " does not know what to do with input." << endl;
	}
	...
	int dN = tmax;
	int tmax2 = pow2(tmax+dN);
	VecDoub in_current(tmax2,0.0);
	VecDoub acorrnode(tmax2,0.0);
	vector<double> acorr(dN,0.0);
...
//					if(ie == inode) node_spike[t] = 1;
	...
	//			currentIO = Jio*mo*K;
			...
			// calculate inode in_current
			currentEE = Jee*dotproduct(EE[inode],nwe,Ne);
			currentEI = Jei*dotproduct(EI[inode],nwi,Ni);
			currentEO = Jeo*dotproduct(EO[inode],nwo,No);
			in_current[t] = currentEE + currentEI + currentEO;

		}

		normal_form(in_current,tmax);
		correl(in_current,in_current,acorrnode);
	...
	for(int i=0;i<dN;i++) acorr[i]/=(double)naverage;
	write_matrix(acorr,dN,"acorr.csv");
